...
- **Permissions**: tool calls surface as Allow / Always allow / Deny dialogs (Agent SDK `canUseTool` bridge with input-scoped persisted rules). A new Codex conversation is read-only until a working folder is selected; with an explicit folder, `default`/`acceptEdits` map to `workspace-write`, `plan` to `read-only`, and `bypassPermissions` to `danger-full-access`.
